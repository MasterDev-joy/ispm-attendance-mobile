...
class _StatsAppBar extends StatelessWidget {
  ...
  const _StatsAppBar({
    required this.accent, required this.role, required this.onBack});
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Container(
          ...
          child: Text(role.roleLabel.toUpperCase(),
...
class _StatsBody extends StatelessWidget {
  ...
  const _StatsBody({
    required this.data, required this.role, required this.accent,
    required this.animCtrl, required this.period,
  });
      ...
      children: [
        ...
        _SectionHeader(
          label: role == UserRole.admin
              ? 'Couverture par cours' : 'Présence par cours',
          accent: accent,
          badge: '${data.perCourse.length}',
        ),
        const SizedBox(height: 10),
        ...data.perCourse.asMap().entries.map((e) =>
            _AnimatedCourseBar(
              course:   e.value,
              accent:   accent,
              animCtrl: animCtrl,
...
class _GlobalSummaryCard extends StatelessWidget {
  ...
  const _GlobalSummaryCard({
    required this.data, required this.accent, required this.role});
...
class _QualityIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (rate >= 0.90) {
      label = 'Excellente assiduité'; color = ISPMColors.green;
      icon  = Icons.star_rounded;
    } else if (rate >= 0.80) {
      label = 'Bonne assiduité';      color = ISPMColors.green;
      icon  = Icons.check_circle_outline_rounded;
    } else if (rate >= 0.60) {
      label = 'Assiduité à améliorer'; color = const Color(0xFFF57C00);
      icon  = Icons.warning_amber_rounded;
    } else {
      label = 'Assiduité insuffisante'; color = ISPMColors.error;
...
class _AnimatedCourseBar extends StatelessWidget {
  ...
  const _AnimatedCourseBar({
    required this.course, required this.accent,
    required this.animCtrl, required this.index,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final anim  = CurvedAnimation(parent: animCtrl,
            ...
            Row(children: [
              ...
              _MiniStat(
                  ...
                  color: course.absentCount > 0
                      ? ISPMColors.error : ISPMColors.white.withOpacity(0.30)),
              const SizedBox(width: 14),
              _MiniStat(
